  ...
  const openEdit = async (f: any) => {
    ...
    if (tab === 'achat') {
      ...
      setFormVente({
        ...
        conditionPaiement: full.conditionPaiement || '',
        delaiPaiement: full.delaiPaiement || '',
          ...
          <div className="rounded-lg border p-3 dark:border-gray-700">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <div>
                ...
                <input className="input" placeholder={tab === 'vente-chimiral' ? 'Ex. 06 62 17 62 92' : 'Ex. 0662 176 292'} value={formVente.telephone} onChange={(e) => setFormVente({ ...formVente, telephone: e.target.value })} />
              </div>
              <div>
                <label className="label">Mail</label>
                <input className="input" placeholder={tab === 'vente-chimiral' ? 'Ex. chimiral2010@gmail.com' : 'Ex. contact@oxyral.ma'} value={formVente.mail} onChange={(e) => setFormVente({ ...formVente, mail: e.target.value })} />
              </div>
            </div>
          </div>
              ...
              {formVente.lignes.map((ligne, i) => (
                <div key={i} className="grid gap-2 rounded-lg bg-gray-50 p-3 dark:bg-gray-800 sm:grid-cols-12">
                  <div className="sm:col-span-2">
                    <label className="label text-xs">{tab === 'vente-chimiral' ? 'Emballage' : 'Code'}</label>
                    <input
                      type="text"
                      className="input text-xs"
                      placeholder={tab === 'vente-chimiral' ? 'Ex. B20L, F200L...' : 'Ex. 01'}
                      value={ligne.code || ''}
