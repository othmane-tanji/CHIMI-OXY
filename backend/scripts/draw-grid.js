...
async function main() {
  ...
  for (let x = 0; x < width; x += 50) {
    svgParts.push(`<line x1="${x}" y1="0" x2="${x}" y2="${height}" stroke="rgba(255,0,0,0.3)" stroke-width="1"/>`);
    svgParts.push(`<text x="${x + 2}" y="15" font-size="10" fill="red">${x}</text>`);
  }

  // Draw horizontal grid lines every 50px
  for (let y = 0; y < height; y += 50) {
    svgParts.push(`<line x1="0" y1="${y}" x2="${width}" y2="${y}" stroke="rgba(0,0,255,0.3)" stroke-width="1"/>`);
    svgParts.push(`<text x="2" y="${y - 2}" font-size="10" fill="blue">${y}</text>`);
  }
